...
export const CharacterCard: React.FC<CharacterCardProps> = ({ character }) => {
  ...
  const displayClass = character.characterClass || character.class || 'Unknown';
  ...
  return (
    ...
    >
      ...
      <div className="p-6 relative">
        ...
        <p className="text-[9px] text-[#00f0ff]/60 font-mono uppercase tracking-[0.3em] mb-2">{displayClass}</p>
        <h3 className="text-xl font-bold font-heading text-white group-hover:text-[#00f0ff] transition-colors mb-4">{character.characterName}</h3>
        
        <div className="grid grid-cols-3 gap-1 mb-6 opacity-40 group-hover:opacity-100 transition-opacity">
          {Object.entries(character.stats).slice(0, 3).map(([key, val]) => (
            <div key={key} className="bg-white/5 p-2 rounded-sm text-center border border-white/5">
              <p className="text-[8px] text-gray-500 uppercase font-mono">{key}</p>
              <p className="text-xs font-bold text-white">{val as number}</p>
            </div>
          ))}
        </div>
